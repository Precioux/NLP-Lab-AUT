...
excel_file_path = './guide-bank-2.xlsx'
...
for index, row in guide.iterrows():
    ...
    if type(shorten_value) == str:
        ...
        for example_num in range(1, 101):
...
for key, value in data_dict.items():
    ...
    if value['slot'] == 1:
        ...
        for k in range(n):
            ...
            print(len(label))
            print('*************************************')
            excel_data.append([value["Check"],i, pattern_id, " ".join(new_sentence), " ".join(label), value["Intent - Eng"]])
            i = i + 1
        ...
        # print('######################################')
        excel_data.append(
             [value["Check"], i, pattern_id, " ".join(new_sentence), " ".join(label), value["Intent - Eng"]])
        i = i + 1
        # for index in range(len(new_sentence)):
        #     s = new_sentence[index]
        #     l = label[index]
...
df_excel = pd.DataFrame(excel_data, columns=['Check', 'sample_id', 'pattern_id', 'text', 'slots', 'intent'])

# Save the DataFrame to an Excel file
excel_output_path = 'output-' + filename + '-1403-07-06.xlsx'
